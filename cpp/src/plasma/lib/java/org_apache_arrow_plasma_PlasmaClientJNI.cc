...
JNIEXPORT jobjectArray JNICALL Java_org_apache_arrow_plasma_PlasmaClientJNI_get(
  ...
  for (int i = 0; i < num_oids; ++i) {
    jbyteArray_to_object_id(env, reinterpret_cast<jbyteArray>(
                            env->GetObjectArrayElement(object_ids, i)), &oids[i]);
  }
...
JNIEXPORT jobjectArray JNICALL Java_org_apache_arrow_plasma_PlasmaClientJNI_wait(
  ...
  for (int i = 0; i < num_oids; ++i) {
    jbyteArray_to_object_id(env, reinterpret_cast<jbyteArray>(env->
                            GetObjectArrayElement(object_ids, i)), &oreqs[i].object_id);
